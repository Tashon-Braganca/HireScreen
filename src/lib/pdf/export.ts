...
export const exportToPDF = (jobTitle: string, candidates: RankedCandidate[]) => {
    ...
    doc.text(`${BRAND_NAME}: Candidate Rank Report`, 14, 22);
    ...
    doc.autoTable({
        ...
        body: tableData,
        headStyles: {
            fillColor: [15, 118, 110], // teal-700 (matches accent)
            fontSize: 10,
            halign: "center",
        },
        columnStyles: {
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(
            `Generated by ${BRAND_NAME} AI • Page ${i} of ${pageCount} `,
    ...
    doc.save(`${BRAND_NAME}_Candidates_${jobTitle.replace(/\s+/g, "_")}.pdf`);
};
